...
<div x-data="{
    ...
    <div class="border-b border-[#E5E7EB] pb-5 mb-8">
        <h1 class="text-2xl font-bold tracking-tight text-[#444444] sm:text-3xl">Search Opportunities</h1>
        <p class="mt-1.5 text-xs text-[#7B7B7B] font-medium font-sans">Find engineering, design, and product internships linked directly to university credit systems.</p>
    </div>
    ...
    <div class="flex flex-col md:flex-row gap-4 bg-white border border-[#E5E7EB] rounded p-4 shadow-soft mb-8">
        <div class="flex-grow relative">
            ...
            <input 
                ...
                @input="currentPage = 1"
                type="text" 
                placeholder="Search position titles, skills, or companies..." 
        ...
        <div class="flex flex-wrap gap-2.5">
            <select x-model="selectedWorkplace" @change="currentPage = 1" class="text-xs bg-[#F8FAFA] border border-[#E5E7EB] rounded p-2 font-medium min-w-[130px]">
                <option value="All">All Locations</option>
                <option value="Remote">Remote Only</option>
                <option value="Hybrid">Hybrid</option>
                <option value="On-site">On-site</option>
            </select>
            <select x-model="selectedType" @change="currentPage = 1" class="text-xs bg-[#F8FAFA] border border-[#E5E7EB] rounded p-2 font-medium min-w-[130px]">
                <option value="All">All Fields</option>
                <option value="Engineering">Engineering</option>
                <option value="Design">Product Design</option>
                <option value="Product">Product PM</option>
            </select>
            <select x-model="sortBy" class="text-xs bg-[#F8FAFA] border border-[#E5E7EB] rounded p-2 font-medium min-w-[140px]">
                <option value="match">Sort by: Match Score</option>
                <option value="stipend">Sort by: Stipend (High)</option>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div class="lg:col-span-8 space-y-6">
            ...
            <div class="flex justify-between items-center text-[11px] text-[#7B7B7B] font-semibold uppercase tracking-wider px-1">
            ...
            <template x-if="paginatedList.length === 0">
                <div class="bg-white border border-[#E5E7EB] rounded p-12 text-center text-[#7B7B7B] space-y-3 shadow-soft">
                    <i class="fa-regular fa-folder-open text-3xl text-zinc-300"></i>
                    <p class="font-bold text-sm text-[#444444]">No internships found</p>
                    <p class="text-xs text-[#7B7B7B]">Adjust your search query or clear location filters to see more results.</p>
                </div>
            </template>
            ...
            <div class="space-y-4">
                <template x-for="item in paginatedList" :key="item.id">
                    <div class="bg-white border border-[#E5E7EB] rounded p-6 shadow-soft hover:border-[#00B1AA] transition-colors relative flex flex-col justify-between">
                        
                        <!-- Header panel details -->
                        <div class="flex justify-between items-start gap-4">
                            <div class="flex items-start gap-4">
                                <div :class="item.logoBg" class="h-12 w-12 rounded text-white font-extrabold flex items-center justify-center text-lg z-10 shrink-0" x-text="item.logo"></div>
                                <div class="space-y-0.5">
                                    <h3 class="font-bold text-base text-[#444444] hover:text-[#00B1AA] transition-colors">
                            ...
                            <button @click="toggleSave(item.id)" class="text-[#7B7B7B] hover:text-[#00B1AA] transition-colors p-1">
                                <i :class="savedIds.includes(item.id) ? 'fa-solid fa-bookmark text-[#00B1AA]' : 'fa-regular fa-bookmark'"></i>
                            </button>
                        </div>
                        ...
                        <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-6 text-xs border-y border-zinc-100 py-3.5">
                            <div>
                                <span class="block text-[10px] font-bold text-[#7B7B7B] uppercase tracking-wider">Salary / Stipend</span>
                                <strong class="text-emerald-600 block mt-0.5" x-text="item.stipend"></strong>
                            </div>
                            <div>
                                <span class="block text-[10px] font-bold text-[#7B7B7B] uppercase tracking-wider">Workplace</span>
                                <span class="text-[#444444] font-medium block mt-0.5" x-text="item.workplace"></span>
                            </div>
                            <div>
                                <span class="block text-[10px] font-bold text-[#7B7B7B] uppercase tracking-wider">Location</span>
                                <span class="text-[#444444] font-medium block mt-0.5 truncate" x-text="item.location"></span>
                            </div>
                            <div>
                                <span class="block text-[10px] font-bold text-[#7B7B7B] uppercase tracking-wider">Duration</span>
                        ...
                        <div class="flex flex-wrap gap-1.5 mt-4">
                            <template x-for="skill in item.skills" :key="skill">
                                <span class="bg-[#F8FAFA] border border-[#E5E7EB] text-[#444444] text-[10px] font-semibold px-2 py-0.5 rounded" x-text="skill"></span>
                            </template>
                        </div>

                        <!-- Footer metadata details -->
                        <div class="border-t border-[#E5E7EB] pt-4 mt-5 flex flex-wrap gap-y-2 justify-between items-center text-[10px] text-[#7B7B7B]">
                            ...
                            <div class="flex gap-2">
                                <button @click="alert('Applied via Interlink Direct Match.')" class="rounded bg-[#00B1AA] hover:bg-[#009c95] text-white text-[11px] font-semibold px-3 py-1 shadow-soft transition-colors">
                                    Quick Apply
                                </button>
                            </div>
                        </div>
        ...
        <aside class="lg:col-span-4 space-y-6">
            ...
            <div class="bg-white border border-[#E5E7EB] rounded p-5 shadow-soft space-y-3.5">
                <h3 class="font-bold text-xs uppercase tracking-wider text-[#444444] border-b border-zinc-100 pb-1.5">Recently Viewed</h3>
                <div class="space-y-3">
                    <template x-for="r in recentlyViewed" :key="r.id">
                        <div class="flex justify-between items-center text-xs">
                            ...
                            <a href="/internships/browse" class="text-[#00B1AA] font-bold">View</a>
                        </div>
                    </template>
                </div>
            </div>
            ...
            <!-- Recommended list -->
            <div class="bg-white border border-[#E5E7EB] rounded p-5 shadow-soft space-y-3.5">
                <h3 class="font-bold text-xs uppercase tracking-wider text-[#444444] border-b border-zinc-100 pb-1.5">Recommended For You</h3>
                <div class="space-y-3">
                    <template x-for="rec in recommended" :key="rec.id">
                        <div class="text-xs">
                            ...
                            <div class="flex justify-between items-center mt-2.5">
                                ...
                                <a href="/internships/browse" class="bg-[#00B1AA] text-white text-[10px] font-bold px-2 py-0.5 rounded">Apply</a>
                            </div>
                        </div>
                    </template>
                </div>
            ...
            <div class="bg-white border border-[#E5E7EB] rounded p-5 shadow-soft space-y-3.5">
                <h3 class="font-bold text-xs uppercase tracking-wider text-[#444444] border-b border-zinc-100 pb-1.5">Placement Checklist</h3>
                <ul class="space-y-3 text-xs text-[#7B7B7B] font-medium leading-relaxed">
                    <li class="flex items-start gap-2.5">
                        ...
                        <span><strong>University Email Sync:</strong> Must be completed to access credit audits.</span>
                    </li>
                    <li class="flex items-start gap-2.5">
                        <i class="fa-solid fa-circle-check text-[#00B1AA] mt-0.5"></i>
                        <span><strong>GitHub Parser Connected:</strong> Reads repositories to match tech stacks.</span>
                    </li>
                    <li class="flex items-start gap-2.5">
                        <i class="fa-solid fa-circle-check text-[#00B1AA] mt-0.5"></i>
                        <span><strong>Official Transcript:</strong> Must upload a verified PDF for grade match scores.</span>
                    </li>
                    <li class="flex items-start gap-2.5">
                        <i class="fa-solid fa-circle-check text-zinc-300 mt-0.5"></i>
                        <span><strong>CPT Syllabus Alignment:</strong> Ensure syllabus is mapped by your DSO/faculty.</span>
                    </li>
                </ul>
            </div>
            ...
            <div class="bg-gradient-to-br from-zinc-900 to-zinc-800 text-white rounded p-5 shadow-soft space-y-4">
                <div class="space-y-1.5">
                    <h3 class="font-bold text-xs uppercase tracking-wider text-[#00B1AA]">Need Visa Sponsorship?</h3>
                    <p class="text-[11px] text-zinc-400 leading-relaxed">
                        Interlink hosts automated CPT offer matching to guarantee immediate university registrar validation. Contact DSO support through our guides portals.
                    </p>
                </div>
                <a href="/how-it-works/faq" class="block text-center text-xs font-bold bg-white text-zinc-900 rounded py-2 hover:bg-zinc-100 transition-colors">
                    Read Compliance FAQ
                </a>
            </div>
