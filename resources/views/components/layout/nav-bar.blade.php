...
<div class="w-full border-b transition-colors duration-300"
    :class="$store.ui.darkMode ? 'bg-gray-900 border-gray-800' : 'bg-white border-zinc-200'">
    <div class="mx-auto flex h-8 max-w-7xl items-center justify-end gap-6 px-6">
        <a href="#" class="text-xs font-medium transition-colors duration-200"
            :class="$store.ui.darkMode ? 'text-gray-400 hover:text-white' : 'text-[#68737D] hover:text-[#17494D]'">Sign
            in</a>
        <a href="#" class="text-xs font-medium transition-colors duration-200"
            :class="$store.ui.darkMode ? 'text-gray-400 hover:text-white' : 'text-[#68737D] hover:text-[#17494D]'">Zendesk
            <div>
                <h3 class="mb-4 text-lg font-bold">Resources</h3>
                <div class="space-y-1">
                    <a href="{{ route('navbarlink.resources.blog') }}"
                        class="block rounded-lg px-2 py-2 text-[15px]">Blog</a>
                    <a href="{{ route('navbarlink.resources.cv-builder') }}"
                        class="block rounded-lg px-2 py-2 text-[15px]">CV Builder</a>
                    <a href="{{ route('navbarlink.resources.resume-tips') }}"
                        class="block rounded-lg px-2 py-2 text-[15px]">Resume Tips</a>
                    <a href="{{ route('navbarlink.resources.interview-preparation') }}"
                        class="block rounded-lg px-2 py-2 text-[15px]">Interview Preparation</a>
                </div>
            </div>
            <div>
                <h3 class="mb-4 text-lg font-bold">Company & Help</h3>
                <div class="space-y-1">
                    <a href="{{ route('navbarlink.companies.partners') }}"
                        class="block rounded-lg px-2 py-2 text-[15px]">Partner Companies</a>
...
<div class="flex items-center gap-4">
    ...
    <button type="button"
        ...
        <svg x-show="!$store.ui.darkMode" x-transition:enter="transition ease-out duration-500"
            x-transition:enter-start="opacity-0 rotate-[-180deg] scale-50"
            x-transition:enter-end="opacity-100 rotate-0 scale-100" x-transition:leave="transition ease-in duration-300"
            x-transition:leave-start="opacity-100 rotate-0 scale-100"
            x-transition:leave-end="opacity-0 rotate-[180deg] scale-50" width="18" height="18" viewBox="0 0 18 18"
            fill="none">
            <path d="M15.5 11.5A7 7 0 016.5 2.5a7 7 0 109 9z" fill="none" stroke="currentColor" stroke-width="1.4"
        ...
        <svg x-show="$store.ui.darkMode" x-transition:enter="transition ease-out duration-500"
            ...
            x-transition:enter-end="opacity-100 rotate-0 scale-100" x-transition:leave="transition ease-in duration-300"
            x-transition:leave-start="opacity-100 rotate-0 scale-100"
            x-transition:leave-end="opacity-0 rotate-[-180deg] scale-50" width="18" height="18" viewBox="0 0 20 20"
            fill="currentColor" class="text-amber-400" style="display:none">
    ...
    <style>
        ...
        .theme-spin {
            animation: theme-spin 0.6s cubic-bezier(0.34, 1.56, 0.64, 1) forwards;
        }
    </style>
    ...
    <div class="relative" @click.outside="langOpen = false">
        <button type="button" @click="langOpen = !langOpen"
            ...
            {{-- Chevron --}}
            <svg class="h-3 w-3 transition-transform duration-200" :class="langOpen ? 'rotate-180' : ''" width="12"
                height="12" viewBox="0 0 12 12" fill="none">
                <path d="M2.5 4.5L6 8L9.5 4.5" stroke="currentColor" stroke-width="1.4" stroke-linecap="round"
    ...
    <div class="flex w-full max-w-6xl gap-12 rounded-b-2xl border px-10 py-8 shadow-xl drop-shadow-lg"
        ...
        <div class="flex-1">
            ...
            <div class="grid grid-cols-2 gap-x-8 gap-y-1">
                <a href="{{ route('navbarlink.home.overview') }}"
                    class="flex items-center gap-2.5 rounded-lg px-2 py-2.5 text-[15px] transition-colors duration-150">Overview</a>
                <a href="{{ route('navbarlink.home.features') }}"
                    class="flex items-center gap-2.5 rounded-lg px-2 py-2.5 text-[15px] transition-colors duration-150">Features</a>
                <a href="{{ route('navbarlink.home.statistics') }}"
                    class="flex items-center gap-2.5 rounded-lg px-2 py-2.5 text-[15px] transition-colors duration-150">Statistics</a>
                <a href="{{ route('navbarlink.home.opportunities') }}"
                    class="flex items-center gap-2.5 rounded-lg px-2 py-2.5 text-[15px] transition-colors duration-150">Latest
                    Opportunities</a>
                <a href="{{ route('navbarlink.home.testimonials') }}"
                    class="flex items-center gap-2.5 rounded-lg px-2 py-2.5 text-[15px] transition-colors duration-150">Testimonials</a>
            </div>
        </div>
        <div class="w-72 shrink-0 border-l pl-8" :class="$store.ui.darkMode ? 'border-gray-700' : 'border-gray-200'">
            <h3 class="mb-4 text-lg font-bold" :class="$store.ui.darkMode ? 'text-white' : 'text-gray-900'"
...
<div x-show="activeDropdown === 'solutions'" @mouseenter="openDropdown('solutions')" @mouseleave="closeDropdown()"
    ...
    <div class="flex w-full max-w-6xl gap-16 rounded-b-2xl border px-10 py-8 shadow-xl drop-shadow-lg"
        :class="$store.ui.darkMode ? 'bg-gray-900 border-gray-800' : 'bg-white border-gray-200'">
        <div>
            <h3 class="mb-4 text-lg font-bold">Explore Internships</h3>
            <div class="space-y-1">
                <a href="{{ route('navbarlink.internships.browse') }}"
                    class="block rounded-lg px-2 py-2 text-[15px]">Browse Internships</a>
                <a href="{{ route('navbarlink.internships.remote') }}"
                    class="block rounded-lg px-2 py-2 text-[15px]">Remote Internships</a>
                <a href="{{ route('navbarlink.internships.on-site') }}"
                    class="block rounded-lg px-2 py-2 text-[15px]">On-site Internships</a>
                <a href="{{ route('navbarlink.internships.hybrid') }}"
                    class="block rounded-lg px-2 py-2 text-[15px]">Hybrid Internships</a>
                <a href="{{ route('navbarlink.internships.paid') }}" class="block rounded-lg px-2 py-2 text-[15px]">Paid
                    Internships</a>
                <a href="{{ route('navbarlink.internships.saved') }}"
                    class="block rounded-lg px-2 py-2 text-[15px]">Saved Internships</a>
                <a href="{{ route('navbarlink.internships.categories') }}"
                    class="block rounded-lg px-2 py-2 text-[15px]">Internship Categories</a>
                <a href="{{ route('navbarlink.internships.tracker') }}"
                    class="block rounded-lg px-2 py-2 text-[15px]">Application Tracker</a>
            </div>
        </div>
    </div>
</div>
...
<div x-show="mobileOpen" x-transition:enter="transition ease-out duration-300"
    ...
    :class="$store.ui.darkMode ? 'bg-gray-950 border-gray-800' : 'bg-white border-gray-200'" class="border-b md:hidden"
    style="display:none">
    <div class="mx-auto max-w-7xl space-y-1 px-6 py-4">
        ...
        {{-- Mobile: Pricing --}}
        <a href="#" :class="$store.ui.darkMode ? 'text-gray-200 hover:bg-white/10' : 'text-gray-900 hover:bg-gray-100'"
