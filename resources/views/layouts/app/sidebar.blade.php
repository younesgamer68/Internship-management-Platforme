...
<body class="min-h-screen bg-zinc-50 dark:bg-zinc-950">
    ...
    <div class="mobile-header lg:hidden fixed top-0 inset-x-0 h-12 bg-black flex items-center px-4 z-40 gap-3 border-b border-zinc-800">
        <button onclick="openMobileSidebar()" class="p-1.5 rounded-lg bg-transparent border-none text-zinc-400 hover:bg-zinc-900 hover:text-zinc-100 transition-colors cursor-pointer">
        ...
        <flux:dropdown position="top" align="end">
            <flux:button variant="ghost" icon-trailing="chevron-down" class="gap-2">
                @if (auth()->user()->avatar)
                    <img src="{{ Storage::url(auth()->user()->avatar) }}" alt="{{ auth()->user()->name }}" class="h-7 w-7 rounded-full object-cover border border-zinc-700">
                @else
                ...
                <flux:menu.radio.group>
                    <div class="p-0 text-sm font-normal">
                        <div class="flex items-center gap-2 px-1 py-1.5 text-start text-sm">
                            @if (auth()->user()->avatar)
                                <img src="{{ Storage::url(auth()->user()->avatar) }}" alt="{{ auth()->user()->name }}" class="h-8 w-8 rounded-full object-cover border border-zinc-700">
                            @else
    ...
    <div id="app-sidebar" x-data="{
        ...
        <nav class="flex-1 flex flex-col gap-1 py-3 overflow-y-auto overflow-x-hidden">
            @if (Auth::user()->isAdmin())
                <div class="px-3 pt-2 pb-1 text-xs font-semibold uppercase tracking-[0.2em] text-zinc-600 sidebar-label">Admin</div>

                <a href="{{ route('app.home') }}" wire:navigate class="mx-3 h-10 flex items-center rounded-lg transition-all duration-200 hover:translate-x-1 no-underline {{ request()->routeIs('app.home', 'dashboard', 'admin.dashboard', 'agent.dashboard') ? 'bg-zinc-800 text-white' : 'text-zinc-400 hover:bg-zinc-900 hover:text-white' }}">
                    <div class="w-10 flex items-center justify-center shrink-0">
                ...
                <a href="{{ route('app.home') }}#companies" wire:navigate class="mx-3 h-10 flex items-center rounded-lg transition-all duration-200 hover:translate-x-1 no-underline text-zinc-400 hover:bg-zinc-900 hover:text-white">
                    <div class="w-10 flex items-center justify-center shrink-0">
                        <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round" viewBox="0 0 24 24">
                            <path d="M3 7h18" />
                            <path d="M6 7v14" />
                ...
                <a href="{{ route('app.home') }}#interns" wire:navigate class="mx-3 h-10 flex items-center rounded-lg transition-all duration-200 hover:translate-x-1 no-underline text-zinc-400 hover:bg-zinc-900 hover:text-white">
                    <div class="w-10 flex items-center justify-center shrink-0">
                        <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round" viewBox="0 0 24 24">
            ...
            @else
                <div class="px-3 pt-2 pb-1 text-xs font-semibold uppercase tracking-[0.2em] text-zinc-600 sidebar-label">Workspace</div>
            ...
            <div class="mt-auto px-3 pb-3">
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <button type="submit" class="mx-3 h-10 w-[calc(100%-1.5rem)] flex items-center rounded-lg transition-all duration-200 hover:translate-x-1 no-underline text-zinc-400 hover:bg-zinc-900 hover:text-white cursor-pointer">
                        <div class="w-10 flex items-center justify-center shrink-0">
                            <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round" viewBox="0 0 24 24">
