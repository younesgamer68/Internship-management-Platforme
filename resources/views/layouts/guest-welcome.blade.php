...
<head>
    ...
    <title>{{ $title ?? 'InterLink System' }}</title>
...
<body x-data
    ...
    <!-- Navigation -->
    <nav class="w-full relative z-50 transition-colors duration-300" :class="$store.ui.darkMode ? 'bg-gray-950/80 backdrop-blur-md border-b border-white/5' :
                    ...
                    <div class="flex items-center gap-3">
                        @auth
                            <a href="{{ url('/dashboard') }}"
                                class="rounded-[12px] bg-[#f79123] px-6 py-2 text-sm font-semibold text-white shadow-sm transition-all duration-200 hover:bg-[#e07d0e] hover:shadow-md">
                                <span x-text="$store.ui.t('heroDashboard') ?? 'Dashboard'"></span>
                            </a>
                        @else
                            <a href="{{ route('choose_path') }}"
                                class="rounded-[12px] bg-[#f79123] hover:bg-[#e07d0e] px-6 py-2 text-sm font-semibold text-white shadow-sm transition-all duration-200 hover:shadow-md">
