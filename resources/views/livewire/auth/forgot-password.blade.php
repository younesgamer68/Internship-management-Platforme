<x-layouts.split-auth title="Reset Password">
    ...
    @if (session('status'))
        <div
            class="mb-4 text-sm font-medium text-green-600 bg-green-50 p-4 rounded-lg border border-green-200 flex items-center gap-2">
    ...
    <form method="POST" action="{{ route('password.email') }}" class="space-y-6" x-data="{ loading: false }"
        ...
        <button type="submit" :disabled="loading"
            ...
            <svg x-show="loading" class="animate-spin -ml-1 mr-3 h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg"
